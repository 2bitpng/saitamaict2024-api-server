...
def get_target(words_set):
  ...
  filtered_words = [word for word in words_set if re.fullmatch(r'[a-z]+', word)]
...
filtered_word_list = [word for word in word_list if re.fullmatch(r'[a-z]+', word)]
...
async def get_problem():
  random_words = random.sample(filtered_word_list,3)
  ...
  while target == None:
    for words in words_list:
      ...
      if new_target == None or new_target in random_words:
        continue
      target = new_target
